    ...
    def _graph_hourglass(self, inputs):
        ...
        with tf.name_scope('model'):
            with tf.name_scope('preprocessing'):
                ...
                r2 = self._residual(pool1, numOut=int(self.nFeats / 2), name='r2')
                r3 = self._residual(r2, numOut=self.nFeats, name='r3')
            # Storage Table
            hg = [None] * self.nStack
            ll = [None] * self.nStack
            ...
            with tf.name_scope('stacks'):
                with tf.name_scope('stage_0'):
                    ...
                    ll[0] = self._conv_bn_relu(hg[0], self.nFeats, 1, 1, 'VALID', name='conv')
                    ll_[0] = self._conv(ll[0], self.nFeats, 1, 1, 'VALID', 'll')

                    out[0] = self._conv(ll[0], self.partnum, 1, 1, 'VALID', name = 'out')
                ...
                for i in range(1, self.nStack - 1):
                    with tf.name_scope('stage_' + str(i)):
                        ...
                        ll[i] = self._conv_bn_relu(hg[i], self.nFeats, 1, 1, 'VALID', name='conv')
                        ll_[i] = self._conv(ll[i], self.nFeats, 1, 1, 'VALID', 'll')

                        out[i] = self._conv(ll[i], self.partnum, 1, 1, 'VALID', name = 'out')
                ...
                with tf.name_scope('stage_' + str(self.nStack - 1)):
                    ...
                    ll[self.nStack - 1] = self._conv_bn_relu(hg[self.nStack - 1], self.nFeats, 1, 1, 'VALID', 'conv')
